...
def get_profile_temp(elapsed, cooling_start_time=None, cooling_start_temp=None):
    ...
    if cooling_start_time is not None and cooling_start_temp is not None:
        ...
        return cooling_start_temp - 0.8 * cooling_elapsed  # -0.8°C per second
            ...
            try:
                ...
                if temp is not None and not math.isnan(temp):
                    ...
                    if current_stage == 5:
                        ...
                        # Calculate target temperature with slope of -0.8°C per second
                        profile_temp = cooling_start_temp - 0.8 * cooling_elapsed
                        print(f"Cooling: {temp}°C, Profile: {profile_temp}°C, Slope: -0.8°C/s")
                    else:

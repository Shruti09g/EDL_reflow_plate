...
def send_nextion_command(command):
    ...
    uart.write(command + b'\xFF\xFF\xFF')
...
def update_display(profile_temp, measured_temp):
    ...
    set_nextion_text("n0", str(int(measured_temp)))
        ...
        if current_time - last_update >= SAMPLE_INTERVAL:
            try:
                ...
                if temp is not None and not math.isnan(temp):
                    ...
                    # Update display with both temperatures
                    update_display(profile_temp, temp)
                    
                    # Check cooling threshold if in stage 5

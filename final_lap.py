...
COOLING_THRESHOLD = 40  # Temperature threshold to turn off cooling
...
pid = PIDController(Kp=100.0, Ki=0.5, Kd=0.0, setpoint=m1)
...
def update_display(profile_temp, measured_temp, elapsed_time):
    ...
    remaining_time = max(0, total_profile_time - elapsed_time)+200
    ...
    send_nextion_command(f'n0.val={int(measured_temp)}')  # Most efficient method
    send_nextion_command(f'n1.val={remaining_time}')
...
def get_calibrated_temp():
    ...
    calibrated_temp = 1.819 * raw_temp - 8.187
    ...
    calibrated_temp += increments * 4.5
        ...
        if current_time - last_update >= SAMPLE_INTERVAL:
            try:
                ...
                if temp is not None and not math.isnan(temp):
                    ...
                    if current_stage == 5:
                        # After profile is complete, continue showing the final profile temp
                        profile_temp = 25 + m1*t1 + m2*t2 + m3*t3 + m4*t4
                    else:
